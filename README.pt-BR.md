# Referências

![Repository language](https://img.shields.io/badge/Language-pt--BR-009C3B)
[![License: MIT](https://img.shields.io/badge/License-MIT-white.svg)](https://github.com/rodolfo-brandao/references/blob/main/LICENSE)

Esta é uma lista de referências relacionadas à computação (e tecnologia em geral), muitas das quais me inspiraram e tiveram grande impacto na minha carreira como programador.

Todas as seções e suas respectivas referências presentes aqui seguem a ordem estabelecida no [documento principal](README.md); escrito e ordenado alfabeticamente em inglês.

> [!NOTE]
> _Minha intenção não é criar uma [awesome list](https://github.com/sindresorhus/awesome), mas simplesmente ter um lugar onde eu possa compartilhar tópicos e pessoas que eu respeito e admiro. Afinal de contas, esta lista não é sobre um tema em específico, mas sobre várias referências que possuem significado **pessoal**. E esse permanece sendo o único propósito._
>
> Aviso: _Alguns links de livros apontam para a Amazon apenas por conveniência. Quero deixar claro que não possuo qualquer afiliação, muito menos recebo comissão por meio deles._

## Sumário
- [Livros](#livros)
    - [Quero Ler](#quero-ler)
- [Pessoas](#pessoas)

## Livros
- [Sistemas Operacionais: Conceitos e Mecanismos (2017)](https://wiki.inf.ufpr.br/maziero/doku.php?id=socm:start) — Autor: Prof. Carlos Maziero

- [The Big Archive: System Design (2023)](https://assets.bytebytego.com/ByteByteGo-Big-Archive-System-Design-2023.pdf) — GRATUITO | Autor: [ByteByteGo](https://bytebytego.com/)

### Quero Ler
- [Introduction to Algorithms, 4th Edition (2022)](https://a.co/d/0eHv1SAq) — Autores: Thomas H. Cormen, Charles E. Leiserson, Ronald L. Rivest, Clifford Stein

- [Storytelling with Data, 1st Edition (2015)](https://a.co/d/00x76cVY) — Autora: Cole Nussbaumer Knaflic

## Pessoas <img src="assets/hot.gif" width="45" />
- [Adolfo Guimarães](https://github.com/adolfoguimaraes) — Pesquisador brasileiro e excelente comunicador, com quem tive o privilégio de estudar durante meu bacharelado em Ciência da Computação e minha pós-graduação em Ciência de Dados. Suas áreas de expertise incluem Machine Learning, Processamento de Linguagem Natural e Visualização de Dados

- [bashbunni](https://github.com/bashbunni) — Entusiasta do open source, descobri seu perfil através de lives na [Twitch](https://www.twitch.tv/bashbunni), onde ela possui uma comunidade incrível. Desde então tenho a seguido e admirado suas habilidades de programação e seus posicionamentos a respeito da área (especialmente sobre privacidade dos dados)

- [Donald Knuth](https://en.wikipedia.org/wiki/Donald_Knuth) — Um dos autores do [algoritmo Knuth–Morris–Pratt (KMP)](https://en.wikipedia.org/wiki/Knuth%E2%80%93Morris%E2%80%93Pratt_algorithm) (aquele por trás do famoso atalho `ctrl`+`f`), ele também é autor da coleção monumental de livros [The Art of Computer Programming](https://en.wikipedia.org/wiki/The_Art_of_Computer_Programming). Acredito que ele seja uma das figuras contemporâneas mais influentes na Ciência da Computação, dadas suas contribuições para a área, que ainda exigem de mim uma certa curva de aprendizado para compreendê-las, mas das quais são fundamentais na vida de um programador

- [Jaron Lanier](https://en.wikipedia.org/wiki/Jaron_Lanier) — Considerado o "pai da realidade virtual", seu ativismo e forte crítica ao atual estado da internet, especialmente sobre redes sociais, me despertam profunda reflexão e admiração pelo seu trabalho

- [John Carmack](https://en.wikipedia.org/wiki/John_Carmack) — Co-fundou a [id Software](https://www.idsoftware.com/en) e desenvolveu um dos meus jogos favoritos de todos os tempos: Quake (1996). Para além disso, contribuiu massivamente para o campo da computação gráfica e ajudou a consolidar o que viria a ser o gênero _First-person Shooter_ (FPS) no universo dos jogos eletrônicos

- [Linus Torvalds](https://github.com/torvalds) — \*Apenas\* a pessoa que criou o kernel Linux e o Git. Dispensa mais apresentações

- [Oleksii Trekhleb](https://github.com/trekhleb) — Engenheiro de software na Uber, alguns dos seus repositórios sobre Python, estrutura de dados e algoritmos tem me ajudado imensamente a consolidar meu conhecimento nesses tópicos

- Thiago de Oliveira — Outro professor com quem tive o privilégio de estudar durante meu bacharelado, quando ele também me orientou na elaboração do meu TCC, e posteriormente durante minha pós-graduação. Suas áreas de especialização incluem Bancos de Dados e _Big Data_, além de possuir ampla experiência de mercado